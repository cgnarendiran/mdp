...
def is_terminal(state):
    x, y = state
    # A = [0 1] and B = [0 2]
    return (x == 0 and y == 1) or (x == 0 and y == 3)
...
# Greedy policy
def compute_state_value_greedy(in_place=False):
    new_state_values = np.zeros((WORLD_SIZE, WORLD_SIZE))
    state_values = new_state_values.copy()
    iteration = 1
    while True:
        ...
        for i in range(WORLD_SIZE):
            for j in range(WORLD_SIZE):
                ...
                for action in ACTIONS:
                    (next_i, next_j), reward = step([i, j], action)
                    values.append(ACTION_PROB * (reward + src[next_i, next_j]))
...
def figure_3_2_epi():
    ...
    plt.savefig('figure_3_2_epi.png')
    plt.close()

def figure_3_5_epi():
    values, sync_iteration = compute_state_value_greedy(in_place=False)
    _, asycn_iteration = compute_state_value_greedy(in_place=True)
    draw_image(np.round(values, decimals=2))
    print('In-place: %d iterations' % (asycn_iteration))
    print('Synchronous: %d iterations' % (sync_iteration))

    plt.savefig('figure_3_5_epi.png')
    plt.close()
